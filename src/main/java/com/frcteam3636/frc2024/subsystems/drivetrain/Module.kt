...
class MAXSwerveModule(
    drivingId: CTREMotorControllerId, turningId: REVMotorControllerId, private val chassisAngle: Rotation2d
) : SwerveModule {
    private val drivingTalon = TalonFX(drivingId).apply {
        configurator.apply(SlotConfigs().withMotorFFGains(DRIVING_FF_GAINS))
        configurator.apply(CurrentLimitsConfigs().withSupplyCurrentLimit(DRIVING_CURRENT_LIMIT))
    }
    ...
    override var desiredState: SwerveModuleState = SwerveModuleState(0.0, -chassisAngle)
        ...
        set(value) {
            ...
            drivingTalon.setControl(
                VelocityVoltage(optimized.speedMetersPerSecond / DRIVING_MOTOR_TRAVEL_PER_REVOLUTION).withSlot(
                    0
                )
            )

            turningPIDController.setReference(optimized.angle.radians, CANSparkBase.ControlType.kPosition)
    ...
    internal companion object Constants {
        ...
        val DRIVING_MOTOR_TRAVEL_PER_REVOLUTION = WHEEL_CIRCUMFERENCE * DRIVING_MOTOR_TO_WHEEL_GEARING;
...
class SimSwerveModule : SwerveModule {
    ...
    private val turningFeedback = PIDController(PIDGains(p = 2.0)).apply {
